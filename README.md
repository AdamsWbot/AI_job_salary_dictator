...
- 本项目采用的是基础的线性回归模型，并使用 L2 正则化降低过拟合风险。
- 由于分类特征较多，one-hot 编码会生成较高维特征，本实现已对低频类别进行合并以减少特征稀疏性。
- 如果你希望进一步提升效果，可以考虑：
  - 使用 Python + scikit-learn 进行更快速迭代
  - 尝试树模型（如随机森林、XGBoost）或神经网络
  - 增加特征工程，例如特征交叉、比例特征、行业/国家类别聚合
